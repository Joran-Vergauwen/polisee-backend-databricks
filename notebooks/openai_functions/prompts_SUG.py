...
You are trying to figure out if the document contains anything about suggestions given by politicians.

// HOW TO FIND
As the document is in Dutch and French, the suggestions are typically preceded by words like “suggestion” in French and “suggestie” in Dutch. 

// COUNTING
Count how many times you find a suggestion mentioned in the document. Pay attention though that the files are in Dutch and French. 

// OUTPUT JSON
Then, after you have made your counts, list a JSON that mentions the all the suggestions. 
For example, if you found 3 suggestions, you should list all of them. 
...
// ADDING THE TOPIC
Furthermore, a suggestion can typically also be linked to a topic. 
Try to assign the most relevant topic to the suggestion as shown in the example output. 
If no topic can be linked, just fill in “/”.
...
Example: "Bart De Wever". Always only capitalize the first letter of the name when referring to any politician. 
...
Try to be as specific as possible, for example, when an issue will be taken into consideration or when an issue will be voted upon, this is more specific than when there is no answer yet. This is the list of possibile statuses: 
...
It’s okay if the JSON gets really long. 
It’s very important to show all of the results in the JSON. 
The JSON will be used for further processing, so it is forbidden to omit any results from the output JSON. 
Do not truncate the output JSON. 
"""