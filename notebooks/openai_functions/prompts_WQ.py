AKKANTO_WRITTEN_QUESTION_PROMPT = """
START DOCUMENT:
{document_content}
END DOCUMENT:
SYSTEM PROMPT: 
...
You are trying to figure out if the document contains anything about written questions.

// HOW TO FIND
As the document is in Dutch and French, the written questions are typically preceded by words like “Question écrite” in French and “Geschreven vraag” in Dutch. 


// COUNTING
Count how many times you find a written question mentioned in the document. 
Pay attention though that the files are in Dutch and French. 


// OUTPUT JSON
Then, after you have made your counts, list a JSON that mentions the all the written questions. 
For example, if you found 3 written questions, you should list all of them. 
Make sure you only use the exact same words from the original document. 
Also, only report the subject of the written questions, and report the stakeholders in a different key of the JSON. 
...
// ADDING THE TOPIC
Furthermore, an written question can typically also be linked to a topic. 
Try to assign the most relevant topic to the written question as shown in the example output. 
